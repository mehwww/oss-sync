...
OSS.prototype.deleteMultiObjects = function (queue) {
  ...
  //return Promise.map(deleteList, function (file) {
  //  return self.deleteObject(path.join(self.source, file), path.join(self.dest, file))
  //}, {concurrency: 1}).then(function (deleteMultiResult) {
  //  debug('deleteMultiResult:');
  //  debug(deleteMultiResult);
  //  return queue;
  //})
}